    ...
    prevBlocks(0)
{
    ...
    {
        setWindowTitle(tr("platinum") + " - " + tr("") + " " + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon(bool fIsTestnet)
{
    ...
    {
        trayIcon->setToolTip(tr("platinum client") + " " + tr("[testnet]"));
...
{
    QString strTitle = tr("platinum"); // default title
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            message(tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid platinum address or malformed URI parameters."),
